...
const ICON_CLASS = 'w-4 h-4 shrink-0 text-vanilla-secondary'
...
function ContactRow({ type, href, wrap = false, children }: { type: 'phone' | 'whatsapp' | 'email' | 'link' | 'location'; href?: string; wrap?: boolean; children: React.ReactNode }) {
  const content = (
    <>
      ...
      <span className={`text-sm font-interTight text-[var(--text-secondary)] ${wrap ? 'break-words' : 'truncate'}`}>{children}</span>
    </>
  )
  const className = "flex items-start gap-2 py-1.5"
  if (href) {
    return (
      <a href={href} className={`${className} hover:text-vanilla-secondary transition-colors`} target="_blank" rel="noopener noreferrer">
        {content}
      </a>
    )
  }
    ...
    <div className="min-h-screen flex bg-[var(--bg-primary)]">
      ...
      <aside className="w-56 shrink-0 border-r border-[var(--border-color)] bg-[var(--bg-secondary)] flex flex-col">
        ...
        <div className="p-6 border-t border-[var(--border-color)] flex items-center gap-2">
          ...
          >
            {theme === 'dark' ? (
              <svg className="w-5 h-5 text-vanilla-secondary" fill="currentColor" viewBox="0 0 20 20">
